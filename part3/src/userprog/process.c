...
load (const char *cmd_line, void (**eip) (void), void **esp)
{
  ...
  *eip = (void (*) (void)) ehdr.e_entry;
  ...
  /* The line below will check if the page is NOT installed. 
     We expect the binary file to be lazily loaded. */
   if (pagedir_get_page (thread_current ()->pagedir, (void *) 0x08048000) != NULL) success = false;
  ...
  while (read_bytes > 0 || zero_bytes > 0) 
    {
      ...
      create_page(file, ofs + page_cnt++ * PGSIZE, upage, page_read_bytes, page_zero_bytes, writable);
